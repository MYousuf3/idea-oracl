...
# Monkey-patch torch.ops to handle missing MoE kernels gracefully
import torch

# Wrap torch.ops._moe_C to catch AttributeError and use PyTorch fallback
_original_moe_C_getattr = None

def _patched_moe_getattr(self, name):
    """Intercept missing MoE kernel calls and provide PyTorch fallback"""
    if name == 'topk_softmax':
        # Provide a PyTorch-based fallback implementation
        def pytorch_topk_softmax(topk_weights, topk_ids, token_expert_indices, num_experts, top_k, K):
            import torch.nn.functional as F
            # Simple PyTorch implementation
            topk_vals, topk_indices = torch.topk(topk_weights, top_k, dim=-1)
            topk_vals_softmax = F.softmax(topk_vals, dim=-1)
            return topk_vals_softmax, topk_indices
        return pytorch_topk_softmax
    
    # For other missing attributes, try original or raise clearer error
    try:
        return getattr(self._original, name)
    except AttributeError:
        raise AttributeError(f"MoE kernel '{name}' not available. Please reinstall vLLM from source with proper compilation.")

# Patch _moe_C if it exists
if hasattr(torch.ops, '_moe_C'):
    original_moe_C = torch.ops._moe_C
    class PatchedMoE:
        def __init__(self, original):
            self._original = original
        
        def __getattr__(self, name):
            return _patched_moe_getattr(self, name)
    
    torch.ops._moe_C = PatchedMoE(original_moe_C)
else:
...
def generate_idea_abstracts(
    ...
    # Create tokenizer to get EOS id, mirroring extract_qas.py style
...
def compute_default_output_path(input_path: str) -> str:
    ...
    try:
        ...
        return os.path.join(base, "finished_data", conf, year, "idea_abstracts.json")
    except ValueError:
        # Fallback: put next to input under finished_data
        base = os.path.dirname(os.path.dirname(norm))
        return os.path.join(base, "finished_data", "idea_abstracts.json")


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Generate 'idea abstracts' from {id, title, abstract, year} records using vLLM and an open-source 20B model."
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to output JSON. Default: finished_data/<conf>/<year>/idea_abstracts.json",
    )
    parser.add_argument(
        "--model",
        default="openai/gpt-oss-20b",
        help="HF model name or path. Default: openai/gpt-oss-20b",
    )
